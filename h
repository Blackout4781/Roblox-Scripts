...
function Notification:MoveUp() --Going to use this to move all the drawing instances up one
    ...
    local distanceUp = Vector2.new(0, -self.NotifGap) --This can be made dynamic but I'm not sure if you'd rather use screen size or an argument up to you

    for _, v in ipairs(Notifications) do
        v:CancelTweens() --Cancel all current tweens that aren't the default
...
function Notification:MoveDown() --Going to use this to move all the drawing instances up one
    ...
    local distanceDown = Vector2.new(0, self.NotifGap) --This can be made dynamic but I'm not sure if you'd rather use screen size or an argument up to you
...
function Notification:Remove()
    table.remove(Notifications, table.find(Notifications, self)) --We kind of want to use this and kind of don't its causing A LOT of issues with a large amount of things, but it also fixes the order issue gl
end
